...
<body>
	...
	<section class="home" id="home">
		<div class="home-text">
			...
			<?php echo "<h1> $name </h1>"; 
			echo "<h3> $service </h3>
			<p>we $service service with 5+ years experience that keep customers <br> coming for about services make best effort.</p>
			<div class='button'>
				<button name='price' class='btn'>Set Price</button>
			</div>";?>
		</div>
	</section>
</form>
	<?php
		...
		$sql="SELECT * FROM `purches_details` WHERE `Provider_mobile`='$mobile_no'";
		...
		while($row>0)
		{
			...
			$sql1="SELECT * FROM `user_acc_data` WHERE `Mobile_No.`=$user_mobile";
	...
	{
		echo "<script>window.open('Change_Price.php','New Window','width=500, height=500, left=500, top=100')</script>";
	}
	?>
	</tbody>
    </table>
